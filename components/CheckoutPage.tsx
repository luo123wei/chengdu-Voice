...
export default function CheckoutPage() {
  ...
  const discount = appliedCoupon
    ? Math.round((subtotal * (appliedCoupon.percent / 100) + Number.EPSILON) * 100) / 100
    : 0;
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const res = await fetch('/api/payment/validate-coupon', {
        ...
        headers: { 'Content-Type': 'application/json' },
        cache: 'no-store',
        body: JSON.stringify({ code: couponCode, email: formData.email, subtotal }),
      });
      const json = await res.json().catch(() => ({}));
      if (res.ok && json.ok) {
        setAppliedCoupon({ code: json.code, percent: json.percent });
        setCouponMsg(`${json.code} applied — you save $${Number(json.discount).toFixed(2)} on this order.`);
      } else {
        setAppliedCoupon(null);
        setCouponError(json.message || 'Invalid discount code.');
      }
    } catch {
      setAppliedCoupon(null);
      setCouponError('Could not verify the code. Please try again.');
    } finally {
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm uppercase tracking-wider disabled:bg-gray-50 disabled:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary/30"
                    />
                    {appliedCoupon ? (
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-sm bg-secondary text-white rounded-lg hover:bg-black transition-colors disabled:opacity-50"
                      >
                        {applyingCoupon ? '...' : 'Apply'}
                      </button>
                    )}
                  </div>
                  {couponMsg && <p className="text-xs text-green-700 mt-1">✓ {couponMsg}</p>}
